...
async function startServer() {
  ...
  const PANCAKE_ROUTER = DEX_ROUTERS['PancakeSwap'];
    ...
    await Promise.all(tokensToScan.map(async (targetToken) => {
      try {
        ...
        if (netProfitBNB > 0n) {
          ...
          if (parseFloat(spreadPercentage) > 0.5) { // Only log spreads above 0.5% to avoid junk
            const scanLog = {
              id: Date.now(),
              type: "MARKET_SCAN",
              asset: targetToken.name,
              ...
              status: "Low Spread (Miss)",
              network: "BSC",
              time: new Date().toLocaleTimeString(),
              hash: "Waiting for >3% spread"
            };
